...
async def _handle_payment_mandate(  # noqa: PLR0913
    ...
    requires input, it verifies the challenge response and completes the
    payment.
...
async def _raise_challenge(
    ...
    have an issuer in the demo, so we raise the challenge here. For
    concreteness, we are using an OTP challenge in this sample.
...
async def _check_challenge_response_and_complete_payment(
    ...
    if _challenge_response_is_valid(challenge_response=challenge_response):
        await _complete_payment(
            payment_mandate, updater, debug_mode, payment_method
        )
        return
...
async def _complete_payment(
    ...
    logging.info(
        'Calling issuer to complete payment for %s with payment credential '
        '%s...',
        payment_mandate_id,
        payment_credential,
    )
...
def _create_payment_receipt(
    ...
    method_name_for_receipt = payment_response.method_name
    return PaymentReceipt(
        ...
        payment_status=Success(
            merchant_confirmation_id=payment_id, psp_confirmation_id=payment_id
        ),
        payment_method_details={'method_name': method_name_for_receipt},
    )
