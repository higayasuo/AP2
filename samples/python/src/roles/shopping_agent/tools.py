...
async def update_cart(
    ...
    if not chosen_cart_id:
        raise RuntimeError(
            'No chosen cart mandate found in tool context state.'
        )
...
async def initiate_payment(tool_context: ToolContext, debug_mode: bool = False):
    ...
    """
    payment_mandate_data = tool_context.state['signed_payment_mandate']
    if not payment_mandate_data:
        raise RuntimeError(
            'No signed payment mandate found in tool context state.'
        )
    # Convert dict back to PaymentMandate object if needed
    payment_mandate = (
        PaymentMandate(**payment_mandate_data)
        if isinstance(payment_mandate_data, dict)
        else payment_mandate_data
    )
    ...
    task = await merchant_agent_client.send_a2a_message(
        outgoing_message_builder
    )
...
def create_payment_mandate(
    ...
    # Convert dict back to CartMandate object if needed
    cart_mandate = (
        CartMandate(**cart_mandate_data)
        if isinstance(cart_mandate_data, dict)
        else cart_mandate_data
    )

    payment_request = cart_mandate.contents.payment_request
    shipping_address_data = tool_context.state['shipping_address']
    # Convert dict back to ContactAddress object if needed
    shipping_address = (
        ContactAddress(**shipping_address_data)
        if isinstance(shipping_address_data, dict)
        else shipping_address_data
    )
