...
async def get_shipping_address(
  ...
  try:
    state_keys = list(tool_context.state.keys()) if hasattr(tool_context.state, 'keys') else "N/A"
    _logger.info("get_shipping_address: tool_context.state keys: %s", state_keys)
    _logger.info("get_shipping_address: Full state: %s", tool_context.state)
  except Exception as e:
    _logger.warning("get_shipping_address: Could not log state details: %s", e)

  shopping_context_id = tool_context.state.get("shopping_context_id")
  if not shopping_context_id:
    try:
      available_keys = list(tool_context.state.keys()) if hasattr(tool_context.state, 'keys') else "Cannot list keys"
    except Exception as e:
  ...
  _logger.info("get_shipping_address: Using shopping_context_id: %s", shopping_context_id)
  ...
  try:
    ...
    _logger.info("get_shipping_address: Sending message to credentials provider...")
    ...
    _logger.info("get_shipping_address: Task artifacts count: %d", len(task.artifacts))

    shipping_address = artifact_utils.only(_parse_addresses(task.artifacts))
    _logger.info("get_shipping_address: Successfully retrieved shipping address: %s", shipping_address)
    return shipping_address
  except Exception as e:
    _logger.error("get_shipping_address: Exception occurred: %s: %s", type(e).__name__, e)
    import traceback
    _logger.error("get_shipping_address: Traceback:\n%s", traceback.format_exc())
    raise
