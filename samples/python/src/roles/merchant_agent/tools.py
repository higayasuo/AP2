...
from roles.merchant_agent import storage
...
async def update_cart(
    ...
    if not cart_mandate:
        await _fail_task(
            updater, f'CartMandate not found for cart_id: {cart_id}'
        )
        return
    ...
    try:
        ...
        tax_and_shipping_costs = [
            PaymentItem(
                label='Shipping',
                amount=PaymentCurrencyAmount(currency='JPY', value=300),
            ),
            PaymentItem(
                label='Tax',
                amount=PaymentCurrencyAmount(currency='JPY', value=250),
            ),
        ]
...
async def initiate_payment(
    ...
    if not processor_url:
        await _fail_task(
            updater,
            f'No payment processor found for method: {payment_method_type}',
        )
        return
    ...
    task = await payment_processor_agent.send_a2a_message(
...
async def dpc_finish(
    ...
    This tool receives the Digital Payment Credential (DPC) response
    (OpenID4VP JSON format), validates it, and simulates payment finalization.
