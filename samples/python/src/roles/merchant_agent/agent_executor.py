...
class MerchantAgentExecutor(BaseServerExecutor):
  ...
  """
    ...
    async def _handle_request(
        ...
        # Check data_parts to determine which tool to use
        payment_mandate = message_utils.find_data_part(
            PAYMENT_MANDATE_DATA_KEY, data_parts
        )
        intent_mandate = message_utils.find_data_part(
            INTENT_MANDATE_DATA_KEY, data_parts
        )

        # If PaymentMandate is present, use initiate_payment directly
        if payment_mandate:
            await tools.initiate_payment(data_parts, updater, current_task)
            return

        # If IntentMandate is present, use find_items_workflow directly
        if intent_mandate:
            await catalog_agent.find_items_workflow(
                data_parts, updater, current_task
            )
            return

        # Otherwise, use the base class method which uses FunctionCallResolver
        await super()._handle_request(
            text_parts, data_parts, updater, current_task
        )
    ...
    async def _validate_shopping_agent(
        ...
        if shopping_agent_id not in _KNOWN_SHOPPING_AGENTS:
            ...
            await _fail_task(
                updater,
                f"Unauthorized Request: Unknown agent '{shopping_agent_id}'.",
            )
            return False
