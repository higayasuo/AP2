...
class IntentMandate(BaseModel):
    ...
    user_cart_confirmation_required: bool = Field(
        True,
        description=(
            "If false, the agent can make purchases on the user's behalf"
            ' once all purchase conditions have been satisfied. This must'
            ' be true if the intent mandate is not signed by the user.'
        ),
    )
...
class CartContents(BaseModel):
    ...
    payment_request: PaymentRequest = Field(
        ...,
        description=(
            'The W3C PaymentRequest object to initiate payment. This contains'
            ' the items being purchased, prices, and the set of payment methods'
...
class PaymentMandateContents(BaseModel):
    ...
    payment_response: PaymentResponse = Field(
        ...,
        description=(
            'The payment response containing details of the payment method'
            ' chosen by the user.'
        ),
    )
...
class PaymentMandate(BaseModel):
    ...
    While the Cart and Intent mandates are required by the merchant to
    fulfill the order, separately the protocol provides additional visibility
    into the agentic transaction to the payments ecosystem. For this purpose,
    the PaymentMandate (bound to Cart/Intent mandate but containing separate
    information) may be shared with the network/issuer along with the standard
    transaction authorization messages. The goal of the PaymentMandate is to
    help the network/issuer build trust into the agentic transaction.
    """
